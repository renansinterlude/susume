...
const totalPages = 73; // Número total de páginas
...
useEffect(() => {
  document.title = "Susume | JLPT N2 - Vocabulário";
}, []);

useEffect(() => {
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/vocabularioN2?page=${currentPage}`);
...
const renderPageButtons = () => {
  ...
  return (
    <div>
      <Cabecalho />
      <h1 className="tituloVerbo"> JLPT N2 - Vocabulário </h1>
      <p className="textoVerbo"> Para o nível N2 são consideradas 1810 palavras como conhecimento básico. </p>
                    ...
                    <Typography>
                      ...
                      <strong>
                        ...
                        {vocabulary["Word-Type"].includes("Verbo") ? (
                          <Link to={`/jlpt-n2/detalhes-palavra-verbo/${vocabulary.Vocabulary}`}>
                            <p className='saibaMais'><u>SAIBA MAIS</u></p>
                          </Link>
                        ) : (
                          <Link to={`/jlpt-n2/detalhes-palavra/${vocabulary.Vocabulary}`}>
