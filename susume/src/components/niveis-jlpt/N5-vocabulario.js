...
const totalPages = 27; // Número total de páginas
...
useEffect(() => {
  document.title = "Susume | JLPT N5 - Vocabulário";
}, []);

useEffect(() => {
  const fetchData = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/vocabularioN5?page=${currentPage}`);
...
const renderPageButtons = () => {
  ...
  return (
    <div>
      <Cabecalho />
      <h1 className="tituloVerbo"> JLPT N5 - Vocabulário </h1>
      <p className="textoVerbo"> Para o nível N5 são consideradas 632 palavras como conhecimento básico. </p>
                    ...
                    <Typography>
                      ...
                      <strong>
                        ...
                        {vocabulary["Word-Type"].includes("Verbo") ? (
                          <Link to={`/jlpt-n5/detalhes-palavra-verbo/${vocabulary.Vocabulary}`}>
                            <p className='saibaMais'><u>SAIBA MAIS</u></p>
                          </Link>
                        ) : (
                          <Link to={`/jlpt-n5/detalhes-palavra/${vocabulary.Vocabulary}`}>
