...
const N1Vocabulario = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/vocabularioN1?page=${currentPage}`);
...
const renderPageButtons = () => {
  const buttons = [];
  const maxPages = Math.ceil(totalPages / maxButtons);
  ...
  return (
    <div>
      <Cabecalho />
      <h1 className="tituloVerbo"> JLPT N1 - Vocabulário </h1>
                    ...
                    <Typography>
                      ...
                      <strong>
                        ...
                        {vocabulary["Word-Type"].includes("Verbo") ? (
                          <Link to={`/jlpt-n1/detalhes-palavra-verbo/${vocabulary.Vocabulary}`}>
                            <p className='saibaMais'><u>SAIBA MAIS</u></p>
                          </Link>
                        ) : (
                          <Link to={`/jlpt-n1/detalhes-palavra/${vocabulary.Vocabulary}`}>
